...
Original file is located at
    https://colab.research.google.com/drive/1SSsG_APjPi-YoGYCmgO1RwFIl1nuNuii
"""
...
def train_one_epoch_mtl(
    ...
    for x, y_seg, y_h, y_b in tqdm(loader, desc=desc):
        ...
        if weighting == "fixed":
            ...
            # Pre-normalize task losses so that scales are comparable inside
            # the uncertainty weighting objective. Raw losses are still logged.
            seg_scale = float(configs.get("uncertainty_seg_scale", 1.0))
            h_scale = float(configs.get("uncertainty_height_scale", 1.0))
            b_scale = float(configs.get("uncertainty_biomass_scale", 1.0))
            norm_losses = torch.stack(
                [
                    task_losses[0] / max(seg_scale, 1e-8),
                    task_losses[1] / max(h_scale, 1e-8),
                    task_losses[2] / max(b_scale, 1e-8),
                ]
            )
...
def visualize_encoder_decoder_features(
    ...
    # Prepare 4x6 grid
    fig, axes = plt.subplots(nrows=4, ncols=6, figsize=(6 * 3.1, 4 * 2.8))
...
configs.update(
    ...
    uncertainty_seg_scale=1.0,
    uncertainty_height_scale=1.0,
    uncertainty_biomass_scale=1.0,
...
history = {
    ...
    "biomass_weight": [],
    "allom_weight": [],
    # Gradient cosine similarities on encoder params
    "cos_seg_h": [],
    "cos_seg_b": [],
    "cos_h_b": [],
}
...
for epoch in range(1, num_epochs + 1):
    ...
    # Gradient cosine similarities (encoder)
