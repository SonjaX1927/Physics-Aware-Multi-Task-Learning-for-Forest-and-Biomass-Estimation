...
def train_one_epoch_mtl(
    ...
    for x, y_seg, y_h, y_b in tqdm(loader, desc=desc):
        ...
        if weighting == "fixed":
            ...
            opt_w.zero_grad()
            # Backprop only to update task weights; retain graph for the main loss backward.
...
def plot_curves(history: dict, task: str = "mtl", target: str = "height"):
    ...
    fig1, axes1 = plt.subplots(1, 5, figsize=(5 * 3.5, 4))
    titles = [
        "Total loss",
        "Seg loss",
        "Height loss",
    ...
    fig2, axes2 = plt.subplots(1, 5, figsize=(5 * 3.5, 4))
...
Multi-task training with three heads (segmentation, height, biomass)
and an allometry-based regularizer between predicted height and biomass.
"""
...
# Optional uncertainty-based weighting parameters
uncertainty_params = None
# Optional GradNorm state
gradnorm_state = None
...
for epoch in range(1, num_epochs + 1):
    ...
    print(
        ...
        f"train h={train_metrics['height_loss']:.4f}, "
        f"train b={train_metrics['biomass_loss']:.4f}, "
        f"train allom={train_metrics['allom_loss']:.4f}, "
        f"val loss={val_metrics['loss']:.4f}, "
        f"val seg={val_metrics['seg_loss']:.4f}, "
        f"val h={val_metrics['height_loss']:.4f}, "
        f"val b={val_metrics['biomass_loss']:.4f}, "
        f"val allom={val_metrics['allom_loss']:.4f}, "
        f"val pixel_acc={val_metrics['pixel_acc']:.4f}, "
        f"val mIoU={val_metrics['mean_iou']:.4f}, "
        f"val mDice={val_metrics['mean_dice']:.4f}, "
        f"val h_R2={val_metrics['height_r2']:.4f}, "
        f"val b_R2={val_metrics['biomass_r2']:.4f}, "
...
if weighting == "fixed":
    print(
        f"[Test weights-fixed] seg={configs['seg_weight']:.3f}, "
        f"h={configs['height_weight']:.3f}, b={configs['biomass_weight']:.3f}; "
        f"allom_weight={configs['allom_weight']:.3f}, "
        f"alpha={configs['allom_alpha']:.3f}, beta={configs['allom_beta']:.3f}, "
        f"min_h={configs['allom_min_height']:.3f}"
    ...
    s_vals = uncertainty_params.detach().cpu().numpy().tolist()
    print(
        "[Test weights-uncertainty] log_sigma2="
        f"seg={s_vals[0]:.3f}, h={s_vals[1]:.3f}, b={s_vals[2]:.3f}; "
        f"allom_weight={configs['allom_weight']:.3f}, "
        f"alpha={configs['allom_alpha']:.3f}, beta={configs['allom_beta']:.3f}, "
        f"min_h={configs['allom_min_height']:.3f}"
    ...
    w_vals = gradnorm_state["weights"].detach().cpu().numpy().tolist()
    print(
        "[Test weights-gradnorm] w_seg={w_vals[0]:.3f}, "
        f"w_h={w_vals[1]:.3f}, w_b={w_vals[2]:.3f}; "
        f"allom_weight={configs['allom_weight']:.3f}, "
        f"alpha={configs['allom_alpha']:.3f}, beta={configs['allom_beta']:.3f}, "
        f"min_h={configs['allom_min_height']:.3f}"
...
# Plot height and biomass joint scatter side-by-side for easier comparison
